...
class QA_ALBERT(BERT):
    """Using ALBERT with Urvashi's weights"""
    def load_tokenizer(self):
#        return AlbertTokenizer.from_pretrained("/scratch/ip04/uk1594/BioBERT/BioBERT-PyTorch/bioasq_albert_finalweights_b4_3e-5_8b")
#        return AlbertTokenizer.from_pretrained("bioasq_albert_finalweights_b4_3e-5_8b")
        return AlbertTokenizer.from_pretrained("bioasq_albert_9btrain9b2test_b4_3e-5_2603_system1")

    def load_bert_model(self):
#        return {'hidden_size': 4096}, AlbertModel.from_pretrained("/scratch/ip04/uk1594/BioBERT/BioBERT-PyTorch/bioasq_albert_finalweights_b4_3e-5_8b")
#        return {'hidden_size': 4096}, AlbertModel.from_pretrained("bioasq_albert_finalweights_b4_3e-5_8b")
        return {'hidden_size': 4096}, AlbertModel.from_pretrained("bioasq_albert_9btrain9b2test_b4_3e-5_2603_system1")
    ...
    def name(self):
        ...
        if self.finetune:
            ...
            str_finetune = "(frozen)"
        return "QA_ALBERT9b%s%s%s" % (str_finetune, str_positions, str_hidden)



class Pos(nn.Module):
    "Use only sentence position"
    def __init__(self, batch_size=32, hidden_layer_size=0, dropout_rate=0.5, 
                 positions=False):
...
#            all_inputs = torch.cat([pooling, positions], 1)
    ...
    def name(self):
        ...
        if self.hidden_layer_size == 0:
            ...
            str_hidden = "-relu(%i)" % self.hidden_layer_size
        return "%s%s" % (str_positions, str_hidden)
    ...
    def fit(self, X_train, Q_train, Y_train,
        ...
        print("Batch size:", self.batch_size)
        print("Input data size:", len(X_positions))
        ...
        for epoch in range(nb_epoch):
            ...
            epoch_loss = 0.0
            batch_count = 0
            progress = progressbar.ProgressBar(max_value=len(X_positions) // self.batch_size,
                                               variables={'loss': '--'},
                                               suffix=' Loss: {variables.loss}',
                                               redirect_stdout=True).start()
            for batch in range(len(X_positions) // self.batch_size):
                f = self.batch_size*batch
                t = self.batch_size*(batch+1)
                input_word_ids = torch.tensor([], device=device)
                input_masks = torch.tensor([], device=device)
                input_segments = torch.tensor([], device=device)
                input_positions = torch.tensor(X_positions[f:t], device=device).float()
                avg_mask = torch.tensor([], device=device)
                targets = torch.tensor(Y_train[f:t], device=device)#.unsqueeze(1)
    ...
    def predict(self, X_topredict, Q_topredict, X_positions=[]):
        ...
        with torch.no_grad():
            ...
            if len(X_positions) % self.batch_size != 0:
                # Remaining data 
                f =  len(X_positions) // self.batch_size * self.batch_size
                input_word_ids = torch.tensor([], device=device)
                input_masks = torch.tensor([], device=device)
                input_segments = torch.tensor([], device=device)
                input_positions = torch.tensor(X_positions[f:], device=device).float()
    ...
    def test(self, X_test, Q_test, Y_test, X_positions=[]):
        ...
        with torch.no_grad():
            for batch in range(len(X_positions) // self.batch_size):
                ...
                avg_mask = torch.tensor([], device=device)
                targets = torch.tensor(Y_test[f:t], device=device)#.unsqueeze(1)
...
#    nnc = BERT(hidden_layer_size=50, positions=True)
#    nnc = BioBERT(hidden_layer_size=50, positions=True)
#    nnc = DistilBERT(hidden_layer_size=50, positions=True, finetune_model=True)   
    nnc = ALBERT(hidden_layer_size=50, positions=True)   
#    nnc = QA_ALBERT(hidden_layer_size=50, positions=True)   
#    nnc = ALBERT_squad2(hidden_layer_size=50, positions=True)   
#    nnc = Pos(hidden_layer_size=50, positions=True)   

    print(nnc.name(), "created")



    labels_dict = rouge_to_labels('rouge_9b.csv', "topn", 5)
    sentences = []
    labels = []
    s_ids = []
    with open('rouge_9b.csv', encoding='utf-8') as f:
