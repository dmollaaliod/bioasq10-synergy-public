...
from multisummarise import sbert_qsummarise
...
for i, q in enumerate(questions):
    ...
    if q['id'] in search_results:
        ...
        for d in result['documents']:
            ...
            single_summary = sbert_qsummarise(question, d['documentAbstract'])
            for s in single_summary:
                ...
                scored_snippets.append((s[1], result_summary))
        scored_snippets.sort(key=lambda s: s[0], reverse=True) # Sort in descending order since the scores are similarities
        #print(scored_snippets[:10])
        snippets = [x[1] for x in scored_snippets]
